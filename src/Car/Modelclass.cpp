...
bool ModelClass::Initialize(ID3D11Device* device, char* modleFile, WCHAR* textureFilename, D3DXVECTOR3* scaleInput)
{
	bool result;
	...
	if (scaleInput == nullptr) {
		...
		scale.z = scaleInput->z;
	}

	// Load in the model data,
	result = LoadModel(modleFile);
	if (!result)
	{
		return false;
	}
...
bool ModelClass::InitializeBuffers(ID3D11Device* device)
{
	...
	{
		vertices[i].position = D3DXVECTOR3(model[i].x * scale.x, model[i].y * scale.y, model[i].z * scale.z);
